...
Metier::Metier(int indexEvt):GenerateurNoeudsProbables (indexEvt)
{
    ...
    case 1 : {
        ...
        m_ConditionSelecteurProba = make_shared<Condition>(0.15 - tmpFavoriseur, p_Relative);
    }break;
    case 2 : {
        m_Nom = Metier::NOBLE_CHEVALIER;
        m_ConditionSelecteurProba = make_shared<Condition>(0 - tmpFavoriseur, p_Relative);
    ...
    case 3 : {
        ...
        m_ConditionSelecteurProba = make_shared<Condition>(0.02 - tmpFavoriseur, p_Relative);
    ...
    if ( m_ConditionSelecteurProba!= nullptr) {
        ...
        m_ConditionSelecteurProba->AjouterModifProba(-2.3,
            {make_shared<Condition>(GenVieHumain::AGE, "180", Comparateur::c_Inferieur),
             make_shared<Condition>(Planete::C_TYPE_PLANETE, Planete::PLANETE_FEODAL, Comparateur::c_Different)
                                       });
    }
...
shared_ptr<Condition> Metier::AjouterModifProbaSiAMetierIntegre(shared_ptr<Condition> cond, double poidsProba)
{
    for ( int i = 0 ; i < Metier::METIERS_INTEGRES.length(); ++i) {
        cond->AjouterModifProba(poidsProba,
            { make_shared<Condition>(Metier::C_METIER, Metier::METIERS_INTEGRES[i], Comparateur::c_Egal) });
    }
    return cond;
}
